...
\end{equation*}
where $m$ is the mass, $c$ is the damping coefficient, and $k$ is the spring
...
high and low ranges can be used to determine to the damping parameter $c$ after
estimating $m$ and $k$.
...
to \autoref{fig:mckfrf}. The solid lines display the frequency response of the
measured data and dashed lines display the frequency response of the
mass-spring-damper system using estimated parameters. Segments are shown in
\autoref{fig:wristdata} by vertical bars and the different segments can be
...
\begin{table}[hb]
    ...
    \begin{tabular}{|r|r|r|r|r|}
        ...
        \hline
    \end{tabular}
    \caption{Error Residual of Parameter Estimates for Wrist Data}
    \label{tab:mckerr}
\end{table}
