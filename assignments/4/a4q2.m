...
for ii=1:4
    ...
    subplot(311)
        p1 = loglog(mfv(est2idx),abs(mH(est2idx)),'x',...
            'color',colset((ii - 1)*3 + 1,:),...
            'linewidth', 4, 'linestyle', ':');
        hold on;
        p2 = loglog(mfv(est2idx),abs(Hid(est2idx)),...
            'color',colset((ii - 1)*3 + 2,:),...
            'linewidth',2,'linestyle','--');
        p3 = loglog(mfv(est2idx),abs(Hid2(est2idx)),...
...
set(gcf, 'PaperPositionMode', 'auto',...
    'Position', [position(1:2) 1400 900*3/2])
l = gridLegend(p, 2,...
    {...
    'segment 1, measured',...
    'segment 1, estimate',...
    'segment 1, improved est.',...
    'segment 2, measured',...
    'segment 2, estimate',...
    'segment 2, improved est.',...
    'segment 3, measured',...
    'segment 3, estimate',...
    'segment 3, improved est.',...
    'segment 4, measured',...
    'segment 4, estimate',...
    'segment 4, improved est.',...
    },...
    'fontsize', 14,...
    'location', 'north')
hgexport(gcf, ['mckfrf', '.eps'],...
...
for ii=1:4
    ...
    subplot(211)
        p1 = loglog(mfv(est2idx),abs(mH(est2idx)),'x',...
            ...
            'linewidth', 4, 'linestyle', ':');
        hold on;
        p2 = loglog(mfv(est2idx),abs(Hid),...
            'color',colset((ii - 1)*3 + 2,:),...
            'linewidth',2,'linestyle','--');
        p3 = loglog(mfv(est2idx),abs(Hid2),...
            'color',colset((ii - 1)*3 + 3,:),...
...
position = get(gcf,'Position');
set(gcf, 'PaperPositionMode', 'auto',...
    'Position', [position(1:2) 1400 900])
l = gridLegend(p, 2,...
    {...
    'segment 1, measured',...
